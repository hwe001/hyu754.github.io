...
const sizeOf = (() => {
  // minimal PNG size reader (avoid extra dep)
  return (buf) => {
...
const ROOT = "/home/user/hyu754.github.io/virtual_twin_pipeline";
...
function parseInline(text, baseOpts = {}) {
  ...
  const flush = (opts = {}) => {
    if (buf.length) {
      runs.push(new TextRun({ text: buf, ...baseOpts, ...opts }));
      buf = "";
    }
  };
  while (i < text.length) {
    if (text.startsWith("**", i)) {
      ...
      if (end !== -1) {
        flush();
        runs.push(new TextRun({ text: text.slice(i + 2, end), bold: true, ...baseOpts }));
        i = end + 2;
        continue;
      }
    }
    if (text[i] === "*" && text[i + 1] !== "*") {
      ...
      if (end !== -1) {
        flush();
        runs.push(new TextRun({ text: text.slice(i + 1, end), italics: true, ...baseOpts }));
        i = end + 1;
        continue;
      }
    }
    if (text[i] === "`") {
      ...
      if (end !== -1) {
        flush();
        runs.push(new TextRun({ text: text.slice(i + 1, end), font: "Courier New", ...baseOpts }));
        i = end + 1;
        continue;
      }
    }
    // [AUTHOR TO COMPLETE] / [AUTHOR TO VERIFY] highlighting
    if (text.startsWith("[AUTHOR TO", i)) {
...
function P(text, opts = {}) {
  return new Paragraph({
    children: parseInline(text, opts.runOpts || {}),
    spacing: { after: 200 },
    ...opts.paraOpts,
  });
}
...
function figureParagraph(filename, maxWidthIn = 6.5) {
  const buf = fs.readFileSync(path.join(FIG, filename));
...
function hr() {
  return new Paragraph({
    text: "",
    border: { bottom: { color: "999999", space: 4, style: BorderStyle.SINGLE, size: 6 } },
    spacing: { before: 200, after: 200 },
  });
}

// ---------------------------------------------------------------------
// Table builder from markdown pipe-table lines
// ---------------------------------------------------------------------
function mdTable(headerCells, rows, colWidthsDxa) {
  ...
  const mkCell = (text, bold, shade) =>
    new TableCell({
      ...
      children: [new Paragraph({ children: parseInline(text, bold ? { bold: true } : {}) })],
    });
  const buildRow = (cells, bold, shade) =>
    new TableRow({
      children: cells.map((c, idx) =>
        new TableCell({
          width: { size: colWidthsDxa[idx], type: WidthType.DXA },
          shading: shade ? { type: ShadingType.CLEAR, fill: "E8E8E8" } : undefined,
          children: [new Paragraph({ children: parseInline(c, bold ? { bold: true } : {}) })],
        })
      ),
    });
  return new Table({
    ...
    rows: [buildRow(headerCells, true, true), ...rows.map((r) => buildRow(r, false, false))],
  });
}
...
const children = [
  ...
  P("Fifth, no experimental or clinical validation of any model output has yet been performed. The internal consistency checks reported here (§3.2's proximity-vs-topology discrepancy, §3.3's independent literature cross-check on terminal density, §3.4's volume-fraction consistency check) are useful but are not a substitute for comparison against measured data — e.g. intraoperative or imaging-derived flow measurements, or histological lobule-density measurements in resected tissue — which the author should pursue before this work is positioned as validated in the sense the special issue calls for."),
  P("Finally, this demonstration is confined to two of eight Couinaud segments in a single liver model; whole-organ scale-up (§4.3) and demonstration across multiple patients are both required before the framework's generality can be claimed."),

  Heading("4.3 Future work", HeadingLevel.HEADING_2),
  P("The immediate priority is resolving the directional-growth limitation of §4.2, since it is a precondition for using this network's geometry in a spatially faithful full-organ or full-territory flow simulation. Beyond that: (i) implementation and validation of the representative-lobule micro-scale layer, closing the macro–meso–micro loop that motivates this framework; (ii) a resolution-convergence study — varying the meso-unit terminal density and tracking a downstream physiological output until it stabilizes — to justify the chosen resolution on model-convergence grounds rather than literature cross-reference alone; (iii) replacement of literature-typical physiological parameters with patient-specific measurements where obtainable, and formal uncertainty quantification over the parameters that remain literature-derived, consistent with this special issue's explicit interest in verification, validation, and uncertainty quantification (VVUQ); (iv) whole-liver scale-up (Table 1 implies approximately 28,700 meso-units at the present terminal density across all eight segments); and (v) extension to the biliary tree, present in the source dataset alongside the vascular trees but not yet incorporated into this framework, which would support a second, independent physiological output (biliary drainage) and a clinically relevant application — flagging vascular and biliary structures placed at risk by a candidate resection plane — not available from vasculature alone."),

  Heading("5. Conclusion", HeadingLevel.HEADING_1),
  P("We present a vascularly constrained meso-unit framework that extends real, topologically-verified portal vasculature into geometrically-confined, hemodynamically-characterized synthetic vascular networks, demonstrated on the clinically relevant Couinaud segments II/III. The framework's geometric and topological properties are independently verified rather than assumed, its terminal resolution is consistent with an independent recent model in the literature, and its outputs are structured explicitly as boundary conditions for a not-yet-implemented lobule-scale physics layer. We report the framework's real current limitations — an unresolved directional-growth artifact, literature-derived rather than measured physiological parameters, and the absence of both the micro-scale layer and experimental validation — as the concrete, prioritized agenda for the work remaining before this framework constitutes a validated virtual twin in the sense this special issue calls for."),

  Heading("Data and code availability", HeadingLevel.HEADING_2),
  P("[AUTHOR TO COMPLETE] All code, compact patient geometry, and computed results underlying this manuscript are available at [repository URL], branch `claude/livermodels-repo-access-ks7sit`, directory `virtual_twin_pipeline/`. Interactive visualizations of the segmented anatomy and the meso-unit vascular network are available at [artifact URLs — confirm intended public accessibility before citing in the submitted manuscript]."),

  Heading("Acknowledgments", HeadingLevel.HEADING_2),
  P("[AUTHOR TO COMPLETE — funding sources, institutional acknowledgments.]"),

  Heading("Author contributions", HeadingLevel.HEADING_2),
  P("[AUTHOR TO COMPLETE — CRediT statement. Note for the author: this manuscript's computational pipeline, verification analyses, and this draft text were produced with substantial AI assistance (Claude, Anthropic); the journal's and institution's policy on disclosure of AI assistance in manuscript preparation should be checked and an appropriate statement added before submission.]"),

  Heading("Conflict of interest", HeadingLevel.HEADING_2),
  P("[AUTHOR TO COMPLETE]"),

  Heading("References", HeadingLevel.HEADING_1),
  new Paragraph({
    children: parseInline("Status note: the entries below were populated via a literature search (live web search, not model memory) and are believed accurate as cited, with two explicit exceptions flagged inline: (i) the blood-viscosity citation (Cho and Kensey, 1991) is a plausible but unconfirmed match for the exact 3.6×10⁻³ Pa·s constant used in §2.5 — verify against the primary CCO literature before submission; (ii) the Strasberg (2005) nomenclature reference was not independently re-verified in a dedicated search pass.", { italics: true, size: 20 }),
    spacing: { after: 200 },
  }),
];

const refs = [
  "1. Broelsch, C.E., Whitington, P.F., Emond, J.C., et al. (1991). Liver transplantation in children from living related donors: surgical techniques and results. Annals of Surgery, 214(4), 428–439.",
  "2. Cho, Y.I., Kensey, K.R. (1991). Effects of the non-Newtonian viscosity of blood on hemodynamics of diseased arterial flows: Part 1, steady flows. Biorheology, 28(3-4), 241–262. [AUTHOR TO VERIFY]",
  "3. Coombe, D., Rezania, V., Tuszynski, J.A. (2022). Dual continuum upscaling of liver lobule flow and metabolism to the full organ scale. Frontiers in Systems Biology, 2, 926923.",
  "4. Couinaud, C. (1957). Le Foie: Études Anatomiques et Chirurgicales. Paris: Masson.",
  "5. Dahm, F., Georgiev, P., Clavien, P.A. (2005). Small-for-size syndrome after partial liver transplantation. American Journal of Transplantation, 5(11), 2605–2610.",
  "6. Eipel, C., Abshagen, K., Vollmar, B. (2010). Regulation of hepatic blood flow: the hepatic arterial buffer response revisited. World Journal of Gastroenterology, 16(48), 6046–6057.",
  "7. Jungermann, K., Kietzmann, T. (1996). Zonation of parenchymal and nonparenchymal metabolism in liver. Annual Review of Nutrition, 16, 179–203.",
  "8. Karch, R., Neumann, F., Neumann, M., Schreiner, W. (1999). A three-dimensional model for arterial tree representation, generated by constrained constructive optimization. Computers in Biology and Medicine, 29(1), 19–38.",
  "9. Kerautret, B., Ngo, P., Passat, N., Talbot, H., Jaquet, C. (2023). OpenCCO: An implementation of constrained constructive optimization for generating 2D and 3D vascular trees. Image Processing On Line, 13, 258–279.",
  "10. Lautt, W.W. (2009). Hepatic Circulation: Physiology and Pathophysiology. San Rafael, CA: Morgan & Claypool Life Sciences.",
  "11. Lorente, S., Hautefeuille, M., Sanchez-Cedillo, A. (2020). The liver, a functionalized vascular structure. Scientific Reports, 10, 16194.",
  "12. Malka-Markovitz, A., Camara Dit Pinto, S., Cherkaoui, M., et al. (2025). Multiscale modeling of drug-induced liver injury from organ to lobule. npj Digital Medicine, 8, 383. (PMC12185720)",
  "13. Rahbari, N.N., Garden, O.J., Padbury, R., et al. (2011). Posthepatectomy liver failure: a definition and grading by the ISGLS. Surgery, 149(5), 713–724.",
  ...
  "15. Strasberg, S.M. (2005). Nomenclature of hepatic anatomy and resections: a review of the Brisbane 2000 system. Journal of Hepato-Biliary-Pancreatic Surgery, 12(5), 351–355. [AUTHOR TO CONFIRM]",
  "16. Vauthey, J.N., Abdalla, E.K., Doherty, D.A., et al. (2002). Body surface area and body weight predict total liver volume in Western adults. Liver Transplantation, 8(3), 233–240.",
];
refs.forEach((r) => children.push(new Paragraph({
  children: parseInline(r, { size: 20 }),
  spacing: { after: 120 },
...
children.push(
  hr(),
  new Paragraph({
    children: parseInline("Draft prepared 2026-09-24. This is a working draft for author review and revision, not a submission-ready manuscript — see [AUTHOR TO COMPLETE] markers throughout, and the accompanying HANDOFF.md for the full technical record this draft is based on.", { italics: true, size: 20 }),
  })
);
...
Packer.toBuffer(doc).then((buf) => {
  fs.writeFileSync(path.join(ROOT, "PAPER_DRAFT.docx"), buf);
  console.log("Wrote PAPER_DRAFT.docx,", buf.length, "bytes");
});
